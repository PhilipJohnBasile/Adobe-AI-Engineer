      ...
      <Flex direction="row" justifyContent="space-between" alignItems="center" marginBottom="size-400">
        ...
        <Flex direction="row" gap="size-150">
          <Button 
            ...
            onPress={async () => {
              try {
                const response = await fetch(`/api/campaigns/${campaign.campaign_id}/generate`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' }
                });
                const result = await response.json();
                if (result.success) {
                  alert(`AI assets generated successfully! Created ${result.assets.length} assets.`);
                } else {
                  alert('Failed to generate assets: ' + result.error);
                }
              } catch (error: any) {
