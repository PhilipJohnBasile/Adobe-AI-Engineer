...
function CampaignDetailV2({ campaignId }: CampaignDetailV2Props) {
  console.log('🚀 CampaignDetailV2 component rendering with campaignId:', campaignId);
  
  const router = useRouter();
  const [campaign] = useState<Campaign | null>(mockCampaign);
  const [loading] = useState(false);
  const [selectedTab, setSelectedTab] = useState('overview');

  console.log('🔍 Render state:', { loading, campaign: !!campaign, campaignId });
      ...
      <View maxWidth="size-7000" marginX="auto" paddingX="size-300" paddingY="size-300">
        ...
        <Flex direction="column" gap="size-300" marginBottom="size-400">
          <Flex alignItems="center" gap="size-200">
            <Button variant="secondary" onPress={() => router.push('/campaigns')}>
              ← Back to Campaigns  
            </Button>
