...
export async function loader({ params }: Route.LoaderArgs): Promise<Route.LoaderData> {
  ...
  try {
    const campaignData = await readCampaign(id);
    const validatedCampaign = CampaignSchema.parse(campaignData);
    
    console.log(`Loaded campaign ${id} for editing`);
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  try {
    ...
    // Save to filesystem using Zod utilities
    await writeCampaign(id, validatedCampaign);
    
    console.log(`Saved campaign ${id} to filesystem`);
