...
export const CampaignForm: React.FC<CampaignFormProps> = ({
    ...
    if (file && campaign?.campaign_id) {
      try {
        ...
        if (response.ok) {
          ...
          setFormData(updatedCampaign);
          setToastMessage('Campaign uploaded successfully');
  ...
  const validateCampaign = () => {
    if (!formData.campaign_name || !formData.products?.length) {
      setToastMessage('Please fill in required fields before validation.');
      setTimeout(() => setToastMessage(null), 3000);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update form state with saved data instead of calling onSave
      setFormData(savedCampaign);
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <View padding="size-300" UNSAFE_style={{ minHeight: '70vh', overflowY: 'auto' }}>
        <Section>
